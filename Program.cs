...
class Program
{
    static void Main()
    {
        ...
        while (!gameOver)
        {
            ...
            Console.WriteLine("Podaj współrzędne (x, y) do odkrycia lub oznaczenia flagą (np. 2 3), poprzedzone 'f' dla flagi (np. f 2 3): ");
            string[] input = Console.ReadLine().Split(' ');

            if (input.Length == 3 && input[0] == "f" && int.TryParse(input[1], out int fx) && int.TryParse(input[2], out int fy))
            {
                if (fx >= 0 && fx < width && fy >= 0 && fy < height)
                {
                    if (displayBoard[fy, fx] == ' ')
                    {
                        displayBoard[fy, fx] = 'F';
                    }
                    else if (displayBoard[fy, fx] == 'F')
                    {
                        displayBoard[fy, fx] = ' ';
                    }
                }
            ...
            else if (input.Length == 2 && int.TryParse(input[0], out int x) && int.TryParse(input[1], out int y))
            {
    ...
    static bool CheckWin(char[,] displayBoard, int bombCount)
    {
        ...
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                if (displayBoard[i, j] != ' ' && displayBoard[i, j] != '*' && displayBoard[i, j] != 'F')
                {
                    uncoveredCount++;
                }
            }
